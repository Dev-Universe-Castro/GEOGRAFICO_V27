...
function showAnalyticsCard(layer) {
    ...
    card.style.boxShadow = '0 3px 14px rgba(0,0,0,0.4)';

    let cardHTML = `
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
            <h6 style="margin: 0;">${layer.name}</h6>
            <div>
                <i class="fas fa-times close-card-btn" data-layer-id="${layer.id}" style="cursor: pointer; color: #888;"></i>
            </div>
        </div>
        <div style="font-size: 11px; color: #555; margin-bottom: 8px;">
            <i class="fas fa-info-circle"></i> ${layer.description || 'Descrição não disponível'}
        </div>
        <div style="font-size: 10px; color: #666; margin-bottom: 5px;">
            <i class="fas fa-layer-group"></i> Tipo: ${layer.type}
        </div>
    `;

    // Add specific details based on layer type
    if (layer.type === 'receitas') {
        cardHTML += `<div style="font-size: 10px; color: #666; margin-bottom: 5px;">
                        <i class="fas fa-tag"></i> Categoria: ${layer.category}
                     </div>`;
    } else if (layer.type === 'revendas') {
        cardHTML += `<div style="font-size: 10px; color: #666; margin-bottom: 5px;">
                        <i class="fas fa-store"></i> Revenda ID: ${layer.revendaId}
                     </div>`;
    }
    ...
    repositionAnalyticsCards();
    console.log(`Card analítico adicionado para camada ${layer.id}`);
}
        ...
        async function loadRevendasLayerForLayer(layer) {
            try {
                ...
                if (data.success) {
                    ...
                    layer.dataColumn = 'value'; // Campo que contém o valor nos dados

                    // Para revendas, todos os municípios têm o mesmo "valor" (presença da revenda)
                    const minMax = { min: 1, max: 1 };
                    layer.minMax = minMax;
                    layer.unit = '';

                    await loadMunicipalityBoundariesForGenericLayer(layer, revendasData, minMax);
                    showAnalyticsCard(layer);
                } else {
                    console.error('Error loading revendas data:', data.error);
                    alert('Erro ao carregar dados de revendas: ' + data.error);
                }
            } catch (error) {
                console.error('Network error:', error);
                alert('Erro de conexão ao carregar dados de revendas');
            }
        }

// Function to apply state filter for a generic layer
function applyStateFilterForLayer(geoData, state) {
    if (!state) {
        return geoData;
    }
